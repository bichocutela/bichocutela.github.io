...
export default function ManagementPanel() {
  ...
  async function refresh(showMessage = false) {
    ...
    try {
      ...
      console.error(error);
      toast.error("Não foi possível carregar os produtos da nuvem. Verifique sua conexão e toque em Atualizar.");
...
function Products({ products, categories, refresh }: { products: ManagedProduct[]; categories: ManagedCategory[]; refresh: (show?: boolean) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    setSelected((current) => new Set(Array.from(current).filter((item) => available.has(item))));
  }, [products]);
    ...
    {!showingResults ? <Empty text="Toque em Todos ou em uma categoria para expandir os produtos."/> : !filtered.length ? <Empty text="Nenhum produto encontrado."/> : <>
      <div className="nrd-management-card-list">
        {groupedVisible.map(([groupName, groupProducts])=><section key={groupName}>
          ...
          <div className="nrd-management-card-list">
            {groupProducts.map((item: ManagedProduct)=><article className="nrd-management-card" key={item.code}>
              <div>
...
function Appearance({ settings, refresh }: { settings: Record<string,unknown>; refresh: (show?: boolean) => Promise<void> }) {
