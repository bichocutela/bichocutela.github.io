...
export default function ProductConsultationEntry() {
  ...
  useEffect(() => {
    const install = () => {
      ...
      const promotionsButton = Array.from(drawer.querySelectorAll<HTMLButtonElement>(".nrd-drawer-link"))
        .find((button) => button.textContent?.trim().includes("Promoções"));
      ...
      promotionsButton.insertAdjacentElement("afterend", button);
    };
    install();
